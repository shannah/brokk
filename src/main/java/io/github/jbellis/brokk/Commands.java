...
public class Commands {
    ...
    public OperationResult cmdAdd(String args) {
        if (args.isBlank()) {
            return OperationResult.error("Please provide filename(s) or fragment index");
        }
        try {
            if (args.trim().matches("\\d+")) {
                var resolved = cm.getFilesFromFragmentIndex(Integer.parseInt(args.trim()));
                cm.addFiles(resolved);
                return OperationResult.success("Added " + resolved);
            }
        } catch (Exception e) {
        ...
        var aggregateFiles = new ArrayList<BrokkFile>();
        for (String token : filenames) {
            var matches = Completions.expandPath(cm.getRoot(), token);
            if (matches.isEmpty()) {
                if (io.confirmAsk("No files matched '" + token + "'. Create?")) {
                    try {
                        ...
                        return OperationResult.error("Error creating " + token + ": " + ex.getMessage());
                    }
                }
            } else {
        ...
        return OperationResult.success("Added " + repoFiles);
    }

    public OperationResult cmdReadOnly(String args) {
        if (args.isBlank()) {
        ...
        try {
            if (args.trim().matches("\\d+")) {
                ...
                return OperationResult.success("Added read-only: " + resolved);
            }
        } catch (Exception e) {
            return OperationResult.error(e.getMessage());
        }
        ...
        return OperationResult.success("Added read-only " + repoFiles);
    }
