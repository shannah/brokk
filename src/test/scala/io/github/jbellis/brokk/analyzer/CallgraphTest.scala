...
class CallgraphTest {
  ...
  def testGetCallgraphTo(): Unit = {
    ...
    assertEquals("a.method1()", callsites("B.callsIntoA").get(0).sourceLine)

    // Expect D.methodD1 calls A.method1
    assertTrue(callsites.contains("D.methodD1"), "Should contain call from D.methodD1")
    assertEquals("a.method1()", callsites("D.methodD1").get(0).sourceLine)
    ...
    assertEquals("this.methodD1()", callsites("D.methodD2").get(0).sourceLine)
  }

  @Test
  def testGetCallgraphFrom(): Unit = {
    val analyzer = getAnalyzer
    val callgraph = analyzer.getCallgraphFrom("D.methodD1", 5)
    ...
    assertEquals("a.method1()", callsites("A.method1").get(0).sourceLine)

    // D.methodD1 calls B.callsIntoA
    assertTrue(callsites.contains("B.callsIntoA"), "Should contain call to B.callsIntoA")
    assertEquals("b.callsIntoA()", callsites("B.callsIntoA").get(0).sourceLine)
  ...
  def testGetCallgraphNonexistent(): Unit = {
    ...
    val emptyCallgraphTo = analyzer.getCallgraphTo("NonExistentClass.method", 5)
    assertEquals(0, emptyCallgraphTo.size(), "Callgraph to non-existent method should be empty")

    val emptyCallgraphFrom = analyzer.getCallgraphFrom("NonExistentClass.method", 5)
  ...
  def testGetCallgraphRecursion(): Unit = {
    ...
    val callgraphFrom = analyzer.getCallgraphFrom("A.method5", 5)
    ...
    val callgraphTo = analyzer.getCallgraphTo("D.methodD1", 5)
  ...
  def testCallCycles(): Unit = {
    ...
    val callgraphFrom = analyzer.getCallgraphFrom("CyclicMethods.methodA", 5)
    ...
    assertEquals("this.methodB()", callgraphFrom.get("CyclicMethods.methodB").get(0).sourceLine)

    // Test the other direction - methodC should show as calling methodA
    val callgraphTo = analyzer.getCallgraphTo("CyclicMethods.methodA", 5)
    assertTrue(callgraphTo.containsKey("CyclicMethods.methodC"), "Should detect methodC calls methodA")
    assertEquals("this.methodA()", callgraphTo.get("CyclicMethods.methodC").get(0).sourceLine)
  }
  ...
  def testGetCallgraphFullyQualified(): Unit = {
    ...
    val callgraphTo = analyzer.getCallgraphTo("io.github.jbellis.brokk.Foo.bar", 5)
    ...
    assertEquals("foo.bar()", callgraphTo.get("UsePackaged.callPackagedMethod").get(0).sourceLine)

    // Test the other direction
    val callgraphFrom = analyzer.getCallgraphFrom("UsePackaged.callPackagedMethod", 5)
    ...
    assertEquals("foo.bar()", callgraphFrom.get("io.github.jbellis.brokk.Foo.bar").get(0).sourceLine)
  }
