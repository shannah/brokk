...
public class IncrementalBlockRendererTest {
    ...
    void multipleMixedCustomTagsAreParsed() {
        ...
        boolean hasCodeBlock = composite.children().stream().anyMatch(c -> c instanceof CodeBlockComponentData);

        boolean hasEditBlock = composite.children().stream().anyMatch(c -> c instanceof EditBlockComponentData);
    ...
    void streamingSessionKeepsComponentInstances() throws Exception {
        SwingUtilities.invokeAndWait(() -> {
            ...
            var codeEditor = ComponentUtils.findComponentsOfType(lastComponent, RSyntaxTextArea.class)
                    .getFirst();
            ...
            assertTrue(
                    ((JEditorPane) root.getComponent(3)).getText().contains("Let's add a second paragraph."),
    ...
    void consecutiveMarkdownBlocksAreMerged() throws Exception {
        ...
        assertTrue(
                ...
                "Markdown blocks must be merged after compaction. Pre-compaction count: " + preMergeCount
                        + ", Post-compaction count: " + postMergeCount);
    ...
    void compactMarkdownIsIdempotent() throws Exception {
        ...
        assertTrue(
                ...
                "Markdown blocks must be merged after first compaction. Pre: " + preMergeCount + ", Post1: "
                        + postMergeCountFirst);
    ...
    void heightDoesNotGrowAndScrollIsPreserved() throws Exception {
        ...
        SwingUtilities.invokeAndWait(() -> {
            ...
            assertSame(firstCompBefore, firstCompAfter, "First child component instance should be reused");
        });
    }
}
