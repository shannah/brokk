...
/**
 * Utility for producing short, log‑friendly summaries from arbitrary text (human prompts, JSON blobs, stack‑traces,
 * file paths, etc.).
 */
public final class LogDescription {
    ...
    /**
     * Return a stable, human‑readable summary that never exceeds {@link #MAX_CHARACTERS} and at most {@code maxWords}
     * tokens.
     */
    public static String getShortDescription(String description, int maxWords) {
        ...
        String cleaned = Normalizer.normalize(description, Normalizer.Form.NFKC).replaceAll("[\\p{Cc}&&[^\\s]]", "");
