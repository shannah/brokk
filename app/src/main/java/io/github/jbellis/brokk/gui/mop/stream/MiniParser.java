...
/**
 * A parser that processes a Jsoup Element tree and extracts custom tags (like code-fence, edit-block) even when they're
 * nested inside regular HTML elements.
 *
 * <p>This creates a "mini-tree" representation per top-level element, allowing the incremental renderer to properly
 * handle nested custom blocks while maintaining its flat list architecture.
 */
public class MiniParser {
        ...
        if (node instanceof Element element) {
            ...
            if (factories.containsKey(tagName)) {
                ...
                try {
                    ...
                    logger.warn("Failed to parse {} tag: {}. Error: {}", tagName, element.outerHtml(), ex.getMessage());
                ...
                element.attributes().forEach(attr -> sb.append(" ")
                        .append(attr.getKey())
                        .append("=\"")
    ...
    /**
     * Remembers the first node that contributes to an empty StringBuilder. This node becomes the "anchor" for
    ...
    /**
     * Computes a stable, deterministic ID for a markdown segment based on the anchor node's position in the document.
     *
