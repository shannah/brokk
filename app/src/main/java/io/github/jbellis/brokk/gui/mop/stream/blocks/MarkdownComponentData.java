...
public record MarkdownComponentData(int id, String html) implements ComponentData {
    ...
    private JEditorPane createHtmlPane(boolean isDarkTheme) {
        ...
        ss.addRule("body { font-family: 'Segoe UI', system-ui, sans-serif; line-height: 1.5; " + "background-color: "
                + bgColorHex + "; color: " + textColorHex + "; margin: 0; padding-left: 8px; padding-right: 8px; }");
        ...
        ss.addRule("tr:nth-child(even) { background-color: "
        ...
        ss.addRule("." + SearchConstants.SEARCH_HIGHLIGHT_CLASS + " { background-color: " + searchColorHex
                + "; color: black; }");
        ss.addRule("." + SearchConstants.SEARCH_CURRENT_CLASS + " { background-color: " + currentSearchColorHex
                + "; color: black; }");
