...
/**
 * Renders markdown content incrementally, reusing existing components when possible to minimize flickering and maintain
 * scroll/caret positions during updates.
 */
public final class IncrementalBlockRenderer {
    ...
    /**
     * Register (or clear) a listener that will be invoked exactly once for every completed rendering pass. Passing
     * {@code null} removes any previously registered listener.
     */
    ...
    public IncrementalBlockRenderer(boolean dark, boolean enableEditBlocks, boolean escapeHtml) {
        ...
        MutableDataSet options = new MutableDataSet()
                .set(Parser.EXTENSIONS, Arrays.asList(TablesExtension.create(), BrokkMarkdownExtension.create()))
    ...
    /**
     * Re-runs the current HtmlCustomizer against the last rendered Markdown and updates the Swing components. Safe to
     * call from any thread. Does nothing if no markdown has been rendered yet.
     */
    ...
    /**
     * Returns false when the current htmlCustomizer can be proven to have no impact on the supplied markdown, allowing
     * us to skip re-rendering.
     */
    ...
    /**
     * Updates the content with the given markdown text. Parses the markdown, extracts components, and updates the UI
     * incrementally.
     *
    ...
    /**
     * Thread-safe version of update that separates parsing from UI updates. This method can be called from any thread,
     * with only the UI update happening on the EDT.
     *
    ...
    public void applyUI(List<ComponentData> components) {
        if (compacted) {
            logger.warn(
                    "[COMPACTION] applyUI skipped - renderer already compacted. Incoming size: {}", components.size());
            return;
        }
    ...
    /**
     * Swing's HTMLEditorKit predates HTML5 and does not recognize &apos;. Convert to the numeric entity &#39; that it
     * does understand.
     */
    ...
    /**
     * Builds a list of component data by parsing the HTML and extracting all placeholders and intervening prose
     * segments.
     *
    ...
    /**
     * Ensures that a single CompositeComponentData produced for a top-level element gets a stable, deterministic id
     * derived from the element's source offset (via IdProvider). If the input contains anything other than one
     * composite, it is returned unchanged.
     *
     ...
     */
    private List<ComponentData> normalizeCompositeId(Element topLevelElement, List<ComponentData> parsed) {
    ...
    /**
     * Builds a snapshot of what the component data would look like if compacted. This method performs CPU-intensive
     * work and should be called off the EDT. It does not modify the renderer's state.
     *
     * @return A list of {@link ComponentData} representing the compacted state, or {@code null} if compaction is not
     *     needed (e.g., already compacted or no content).
     */
    ...
    /**
     * Applies a previously built compacted snapshot to the UI. This method must be called on the EDT. It updates the
     * renderer's state.
     *
     * @param mergedComponents The list of {@link ComponentData} from {@link #buildCompactedSnapshot(long)}. If
     *     {@code null}, it typically means compaction was skipped or not needed.
     */
    ...
    /**
     * Updates the style of a specific marker by its ID using CSS classes. This method performs regex replacement to
     * avoid regenerating all marker IDs.
     */
