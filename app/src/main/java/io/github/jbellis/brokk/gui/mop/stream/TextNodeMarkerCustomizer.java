...
/**
 * HtmlCustomizer that wraps every appearance of a given term occurring in normal text nodes with configurable start/end
 * wrappers. Works at the DOM level, leaving existing markup untouched.
 */
public final class TextNodeMarkerCustomizer implements HtmlCustomizer {
    ...
    /**
     * Attribute used to mark wrapper elements created by this customizer. Any text inside an element bearing this
     * attribute will be ignored on subsequent traversals, preventing repeated wrapping.
     */
    ...
    /**
     * Global id generator for {@link #BROKK_ID_ATTR}. Thread-safe, simple monotonic counter. We deliberately do not
     * reset between customizer instances because we only need *uniqueness* inside a single JVM session.
     */
    ...
    public TextNodeMarkerCustomizer(
            String term, boolean caseSensitive, boolean wholeWord, String wrapperStart, String wrapperEnd) {
    ...
    /**
     * Fast check used by renderers to see if this customizer could possibly influence the supplied text. Returns
     * {@code true} if the term occurs at least once according to the current configuration.
     *
    ...
    private class Visitor implements NodeVisitor {
        ...
        /**
         * Returns true if the node has an ancestor element that already carries the custom wrapper attribute, meaning
    ...
    /**
     * Apply multiple highlights to a single text node. Ranges are already sorted by start position from the matcher.
     */
