...
public record CodeBlockComponentData(int id, String body, String lang) implements ComponentData {
    ...
    public void updateComponent(JComponent component) {
        ...
        var textAreas =
                ComponentUtils.findComponentsOfType(component, org.fife.ui.rsyntaxtextarea.RSyntaxTextArea.class);
        if (!textAreas.isEmpty()) {
            var textArea = textAreas.getFirst();

            int currentLen = Optional.ofNullable((Integer) textArea.getClientProperty(LAST_LEN_KEY))
                    .orElse(0);

            boolean looksLikeAppend = false;
            try {
                looksLikeAppend = body.length() >= currentLen && body.startsWith(textArea.getText(0, currentLen));
            ...
            if (looksLikeAppend) {
                ...
                if (!delta.isEmpty()) {
                    ...
                    int sinceDiscard = Optional.ofNullable((Integer) textArea.getClientProperty(SINCE_DISCARD_KEY))
                                    .orElse(0)
                            + delta.length();
    ...
    /**
     * Creates a code block panel using BaseChatMessagePanel. This provides consistent styling with other message
     * components.
     *
    ...
    private JPanel createMessageBubble(RSyntaxTextArea textArea, String fenceInfo, boolean isDarkTheme) {
        ...
        String title = fenceInfo.isEmpty()
                ? "Code"
