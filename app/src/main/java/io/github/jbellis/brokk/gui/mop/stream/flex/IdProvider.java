 ...
 *
 * <p>These IDs are used to track components across re-renders, allowing the incremental renderer to reuse existing
 * Swing components when their content hasn't changed.
 */
public class IdProvider {
    ...
    /**
     * Generates a stable ID for a node based on its source position. This ensures that the same block of text will
     * always get the same ID, even if surrounding content changes.
     *
    ...
    /**
     * Generates a stable ID for a JSoup Element based on its string representation and position in the document.
     *
